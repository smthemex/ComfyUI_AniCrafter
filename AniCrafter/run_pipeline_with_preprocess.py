...
def crop_image(image_list, max_frames=81):
    return image_list[:max_frames] if len(image_list) > max_frames else image_list
...
def predata_for_anicrafter(frame_process_norm,image_list,character_image,AniCrafter_weigths_path,max_frames= 81):
    W, H = image_list[0].size
    H, W = math.ceil(H / 16) * 16, math.ceil(W / 16) * 16
    file_prefix = datetime.now().strftime('%Y%m%d_%H%M%S')
     # Step 1: Parsing human masks
    
    assert image_list is not None and len(image_list) > 1
    mask_list=get_hunman_mask(image_list) # 可以调用lhm_runner里的seg

    # Step 2: Estimating SMPLX parameters and rendering SMPLX mesh video
    SMPLX_MODEL=prepare_smplx_model(AniCrafter_weigths_path)
    smplx_mesh_list,smplx_path=get_smplx_mesh(image_list,mask_list,folder_paths.get_output_directory())
    smplx_json_paths = sorted(os.path.join(smplx_path, x) for x in os.listdir(smplx_path))[:max_frames]

    scene_path=folder_paths.get_output_directory() #TODO

    smplx_mesh_path = scene_path.replace('raw_video', 'smplx_video')

    # Step 3: Background inpainting
    mask_dilation = 16 #TODO
    ref_stride = 2
    neighbor_length = 3
    resize_ratio = 0.5
    pretrain_model_url = 'https://github.com/sczhou/ProPainter/releases/download/v0.1.0/'
    bkgd_video_list=InferencePropainter(pretrain_model_url,image_list,mask_list,mask_dilation,ref_stride,neighbor_length,resize_ratio)


    # step 4: Estimating LRM parameters and rendering LRM video
    lhm_runner = HumanLRMInferrer()
   
    character_image_path = os.path.join(folder_paths.get_temp_directory(), f"AniCrafter_{file_prefix}.jpg")
    character_image.save(character_image_path)
    save_gaussian_path=character_image_path.replace('.jpg', '_gaussian.pth')

    gaussians_list, body_rgb_pil, crop_body_pil = lhm_runner.infer(character_image_path, save_gaussian_path)
    ...
    bkgd_pils_origin = crop_image(bkgd_video_list, max_frames=max_frames)
    smplx_mesh_pils_origin = crop_image(smplx_mesh_list,  max_frames=max_frames)
    ...
    smplx_mesh_pils_origin = crop_image(smplx_mesh_path)
    ...
    return ref_combine_blend_tensor,ref_combine_smplx_tensor,ref_frame,H, W,ref_frame_pils_origin





def infer_anicrafter(pipe, ref_combine_blend_tensor,ref_combine_smplx_tensor,height,width,prompt,negative_prompt,num_inference_steps,seed,use_teacache,cfg_value,use_tiled):
