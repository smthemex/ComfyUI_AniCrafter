...
# sys.path.append(current_dir_path + "/../ProPainter")
...
def read_frame_from_videos(frame_root):
    if frame_root.endswith(('mp4', 'mov', 'avi', 'MP4', 'MOV', 'AVI')): # input video path
        ...
        size = frames[0].size
        return frames, fps, size, video_name, None
        ...
        size = frames[0].size
        return frames, fps, size, video_name, fr_lst
...
def read_mask(masks_img, length, size, flow_mask_dilates=8, mask_dilates=5):
    #masks_img = []
    masks_dilated = []
    flow_masks = []
    ...
    # masks_img, _, _ = torchvision.io.read_video(filename=mnames, pts_unit='sec') # RGB
    # masks_img = list(masks_img.numpy())
    # masks_img = [Image.fromarray(f) for f in masks_img]
...
def InferencePropainter(pretrain_model_url,gt_list,mask_list,mask_dilation,ref_stride,neighbor_length,resize_ratio):
    ...
    #scene_roots = sorted([os.path.join(root, x) for x in os.listdir(root)])

    #for gt_video in tqdm(scene_roots):

    #scene_name = os.path.basename(gt_video).split('.')[0]
    #mask_video = gt_video.replace('raw_video', 'mask_video')
   ...
   # os.makedirs(os.path.dirname(save_path), exist_ok=True)
    ...
    #frames, fps, size, video_name, _ = read_frame_from_videos(gt_video)
    size=gt_list[0].size
    ...
    frames, size, out_size = resize_frames(gt_list, size)

    if not args.resize_ratio == 1.0:
        out_size = origin_size
    
    # fps = args.save_fps if gt_fps is None else gt_fps
    ...
    if args.mode == 'video_inpainting':
        ...
        flow_masks, masks_dilated = read_mask(mask_list, frames_len, size, 
