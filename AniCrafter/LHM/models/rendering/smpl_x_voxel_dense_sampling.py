...
from .smplx.smplx.lbs import blend_shapes
from .smplx.vis_utils import render_mesh
import folder_paths
human_model_path=os.path.join(folder_paths.models_dir, "AniCrafter/pretrained_models/human_model_files")
...
class SMPLX_Mesh(object):
    ...
    def register_constrain_prior(self):
        ...
        constrain_body = np.load(
             os.path.join(folder_paths.models_dir, "AniCrafter/pretrained_models/voxel_grid/human_prior_constrain.npz")
        )["masks"]
...
class SMPLXVoxelMeshModel(nn.Module):
    ...
    def dense_sample(self, body_face_ratio, dense_sample_points):
        ...
        buff_path = f"{base_model_path}/pretrained_models/dense_sample_points/{self.cano_pose_type}_{dense_sample_points}.ply"
    ...
    def voxel_skinning_init(self, scale_ratio=1.05, voxel_size=256):
        ...
        if os.path.exists(f"{base_model_path}/pretrained_models/voxel_grid/voxel_{voxel_size}.pth"):
            print(f"load voxel_grid voxel_{voxel_size}.pth")
            voxel_flat = torch.load(
                os.path.join(f"{base_model_path}/pretrained_models/voxel_grid/voxel_{voxel_size}.pth"),
