...
class RealESRGANer:
    ...
    def __init__(
        ...
        if isinstance(model_path, list):
            ...
            loadnet = torch.load(model_path,weights_only=False, map_location=torch.device("cpu"))
...
class ESRGANEasyModel:
    def __init__(
        self, model_path=f"{base_model_path}/pretrained_models/RealESRGAN_x4plus.pth", face_enhance=True
    ):
        model = RRDBNet(
            num_in_ch=3,
            num_out_ch=3,
        ...
        if self.face_enhance:  # Use GFPGAN for face enhancement
            ...
            self.face_enhancer = GFPGANer(
                model_path=f"{base_model_path}/pretrained_models//GFPGANv1.3.pth", #"https://github.com/TencentARC/GFPGAN/releases/download/v1.3.0/GFPGANv1.3.pth"
                upscale=4,
                arch="clean",
