...
class WanMovieCrafterVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        randomref_dim = 20
        self.randomref_emb_blend = nn.Sequential(
        ...
        self.randomref_emb_smplx = nn.Sequential(
            nn.Conv2d(3, concat_dim * 4, 3, stride=1, padding=1),
            nn.SiLU(),
            nn.Conv2d(concat_dim * 4, concat_dim * 4, 3, stride=1, padding=1),
            nn.SiLU(),
        ...
        state_dict_new = {}
        for key in model_manager.state_dict_new_module:
            if "randomref_emb_blend" in key:
                state_dict_new[key.split("randomref_emb_blend.")[1]] = model_manager.state_dict_new_module[key]
                has_key = True
        self.randomref_emb_blend.load_state_dict(state_dict_new,strict=True)
        assert has_key

        has_key = False
        state_dict_new = {}
        for key in model_manager.state_dict_new_module:
            if "randomref_emb_smplx" in key:
                state_dict_new[key.split("randomref_emb_smplx.")[1]] = model_manager.state_dict_new_module[key]
                has_key = True
        self.randomref_emb_smplx.load_state_dict(state_dict_new,strict=True)
        assert has_key
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None):
        ...
        pipe = WanMovieCrafterVideoPipeline(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            noise_pred_posi = model_fn_wan_video(self.dit, latents, timestep=timestep, **prompt_emb_posi, **image_emb, **extra_input, **tea_cache_posi, add_condition = condition)
