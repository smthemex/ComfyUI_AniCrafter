...
class WanMovieCrafterZeroConvVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        enc_dim = 8
        self.encode_blend_latent = nn.Sequential(
            nn.Conv3d(16, enc_dim * 4, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
            nn.Conv3d(enc_dim * 4, enc_dim * 4, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
            nn.Conv3d(enc_dim * 4, enc_dim * 4, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
            nn.Conv3d(enc_dim * 4, enc_dim * 4, kernel_size=3, stride=1, padding=1),
            ...
            nn.Conv3d(enc_dim * 4, enc_dim * 4, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
            zero_module(nn.Conv3d(enc_dim * 4, 16, kernel_size=3, stride=1, padding=1)),
            )
        ...
        for key in model_manager.state_dict_new_module:
            # print(key)
            if "encode_blend_latent" in key:
                state_dict_new[key.split("encode_blend_latent.")[1]] = model_manager.state_dict_new_module[key]
                has_key = True
        self.encode_blend_latent.load_state_dict(state_dict_new,strict=True)
        assert has_key
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None):
        ...
        pipe = WanMovieCrafterZeroConvVideoPipeline(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        self.encode_blend_latent.to(self.device)
        # self.randomref_emb_blend.to(self.device)
        # self.randomref_emb_smplx.to(self.device)
        blend_data = blend_data.unsqueeze(0)
        smplx_data = smplx_data.unsqueeze(0)

        print(blend_data.shape)

        self.vae.cuda()

        blend_latents = self.vae.encode(blend_data.to(torch.bfloat16), 'cuda', **tiler_kwargs)[0].to(dtype=self.torch_dtype, device=self.device)
        blend_latents = blend_latents.unsqueeze(0)

        # blend_data = self.blend_embedding((torch.cat([blend_data[:,:,:1].repeat(1,1,3,1,1), blend_data], dim=2)/255.).to(self.device)).to(torch.bfloat16)
        # smplx_data = self.smplx_embedding((torch.cat([smplx_data[:,:,:1].repeat(1,1,3,1,1), smplx_data], dim=2)/255.).to(self.device)).to(torch.bfloat16)
        # ref_blend_data = self.randomref_emb_blend((ref_blend_data.unsqueeze(0)/255.).to(self.device).permute(0,3,1,2)).unsqueeze(2).to(torch.bfloat16) # [1, 20, 104, 60]
        # ref_smplx_data = self.randomref_emb_smplx((ref_smplx_data.unsqueeze(0)/255.).to(self.device).permute(0,3,1,2)).unsqueeze(2).to(torch.bfloat16) # [1, 20, 104, 60]
        

        blend_data = self.blend_embedding((torch.cat([blend_data[:,:,:1].repeat(1,1,3,1,1), blend_data], dim=2)).to(self.device)).to(torch.bfloat16)
        smplx_data = self.smplx_embedding((torch.cat([smplx_data[:,:,:1].repeat(1,1,3,1,1), smplx_data], dim=2)).to(self.device)).to(torch.bfloat16)
        # ref_blend_data = self.randomref_emb_blend(ref_blend_data.unsqueeze(0).to(self.device)).unsqueeze(2).to(torch.bfloat16)
        # ref_smplx_data = self.randomref_emb_smplx(ref_smplx_data.unsqueeze(0).to(self.device)).unsqueeze(2).to(torch.bfloat16)


        print(blend_latents.shape)
        print(latents.shape)
        print(blend_data.shape)
        print(smplx_data.shape)
        # print(ref_blend_data.shape)
        # print(ref_smplx_data.shape)

        blend_latents_enc = self.encode_blend_latent(blend_latents.to(self.device).float()).to(torch.bfloat16)  # [1, 16, 21, 104, 60]

        # image_emb["y"] = image_emb["y"]  + ref_blend_data + ref_smplx_data
        # image_emb["y"] = image_emb["y"]
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            model_input = latents + blend_latents_enc
