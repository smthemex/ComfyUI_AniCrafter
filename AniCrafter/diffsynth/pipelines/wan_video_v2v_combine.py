...
class WanMovieCrafterCombineVideoPipeline_(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        self.blend_embedding = nn.Sequential(
            nn.Conv3d(in_ch_emb, concat_dim * 4, (3,3,3), stride=(1,1,1), padding=(1,1,1)),
            nn.SiLU(),
        ...
        self.smplx_embedding = nn.Sequential(
            nn.Conv3d(in_ch_emb, concat_dim * 4, (3,3,3), stride=(1,1,1), padding=(1,1,1)),
            nn.SiLU(),
            nn.Conv3d(concat_dim * 4, concat_dim * 4, (3,3,3), stride=(1,1,1), padding=(1,1,1)),
            nn.SiLU(),
        ...
        for key in model_manager.state_dict_new_module:
            if "blend_embedding" in key:
                state_dict_new[key.split("blend_embedding.")[1]] = model_manager.state_dict_new_module[key]
                has_key = True
        self.blend_embedding.load_state_dict(state_dict_new, strict=True)
        ...
        state_dict_new = {}
        for key in model_manager.state_dict_new_module:
            if "smplx_embedding" in key:
                state_dict_new[key.split("smplx_embedding.")[1]] = model_manager.state_dict_new_module[key]
                has_key = True
        self.smplx_embedding.load_state_dict(state_dict_new, strict=True)
        assert has_key
    ...
    def encode_image_with_blend(self, image, num_frames, height, width, blend_latents, smplx_mask):
        image = self.preprocess_image(image.resize((width, height))).to(self.device)
        clip_context = self.image_encoder.encode_image([image])
        ...
        smplx_mask = smplx_mask.reshape(1, smplx_mask.shape[1] // 4, 4, height // 8, width // 8)  # [1, 20, 4, h//8, w//8]
        msk = torch.ones(1, 1, 4, height//8, width//8, device=self.device)  # [1, 1, 4, h//8, w//8]
        msk = torch.cat([msk, smplx_mask], dim=1).transpose(1, 2)  # [1, 4, 21, h, w]

        # print(msk.shape)
        # print(blend_latents.shape)

        blend_latents = torch.concat([msk, blend_latents.to(device=self.device)], dim=1)  # [1, 4+16, 21, h, w]
        clip_context = clip_context.to(dtype=self.torch_dtype, device=self.device)
        blend_latents = blend_latents.to(dtype=self.torch_dtype, device=self.device)

        return {"clip_feature": clip_context, "y": blend_latents}
    
    def encode_image_with_blend_warpper(self, clip_context, num_frames, height, width, blend_latents, smplx_mask):
    ...
    def encode_image_only_with_mask(self, image, num_frames, height, width, smplx_mask):
        image = self.preprocess_image(image.resize((width, height))).to(self.device)
        clip_context = self.image_encoder.encode_image([image])
        ...
        y = torch.concat([msk, y], dim=1)  # [1, 4+16, 21, h, w]
        clip_context = clip_context.to(dtype=self.torch_dtype, device=self.device)
        y = y.to(dtype=self.torch_dtype, device=self.device)
    ...
    def __call__(
        self,
        prompt, 
        ref_combine_blend_tensor, 
        ref_combine_smplx_tensor, 
        input_image, 
        negative_prompt="",
        input_video=None,
        ...
        if input_video is not None:
            ...
            if not self.uselcm :
                ...
                latents = self.scheduler.add_noise(latents, noise, self.scheduler.sigmas[0])
        else:
            latents = noise
        ...
        # print(ref_combine_blend_tensor.is_cuda)
        # print(ref_combine_smplx_tensor.is_cuda)
        # 
        self.blend_embedding.to(self.device)
        self.smplx_embedding.to(self.device)
    
        # 
        ref_combine_blend_tensor = ref_combine_blend_tensor.unsqueeze(0)
        ref_combine_smplx_tensor = ref_combine_smplx_tensor.unsqueeze(0)

        self.vae.cuda()

        combine_blend_latents = self.vae.encode(ref_combine_blend_tensor.to(torch.bfloat16), 'cuda', **tiler_kwargs)[0].to(dtype=self.torch_dtype, device=self.device)
        combine_blend_latents = combine_blend_latents.unsqueeze(0)


        blend_embedding_input = torch.cat([ref_combine_blend_tensor[:,:,:1].repeat(1,1,3,1,1), ref_combine_blend_tensor], dim=2).to(self.device)
        smplx_embedding_input = torch.cat([ref_combine_smplx_tensor[:,:,:1].repeat(1,1,3,1,1), ref_combine_smplx_tensor], dim=2).to(self.device)



        smplx_mask = (ref_combine_smplx_tensor[:, :, 1:] <= -0.99).all(dim=1, keepdim=False).float()  # [1, 80, h, w]
        combine_mask = torch.ones(1, 1, num_frames, height, width, device=ref_combine_smplx_tensor.device, dtype=ref_combine_smplx_tensor.dtype)
        combine_mask[:, :, 1:] = smplx_mask.unsqueeze(1)  # [1, 1, 80, h, w]
        combine_mask = torch.cat([combine_mask[:, :, :1].repeat(1,1,3,1,1), combine_mask], dim=2)

        # print(combine_mask.shape)
        blend_embedding_input = torch.cat([combine_mask, blend_embedding_input], dim=1)
        smplx_embedding_input = torch.cat([combine_mask, smplx_embedding_input], dim=1)

        smplx_mask = F.interpolate(smplx_mask, scale_factor=1/8, mode='nearest')  # [1, 80, h//8, w//8]

        kernel_size = min(height, width) // 16
        if kernel_size % 2 == 0:
            kernel_size += 1
        sigma = 0.3 * ((kernel_size - 1) * 0.5 - 1) + 0.8
        smplx_mask = torchvision.transforms.functional.gaussian_blur(smplx_mask.permute(1, 0, 2, 3), kernel_size=kernel_size, sigma=sigma).permute(1, 0, 2, 3)

        blend_data = self.blend_embedding(blend_embedding_input).to(torch.bfloat16)
        smplx_data = self.smplx_embedding(smplx_embedding_input).to(torch.bfloat16)
        # print(smplx_data.is_cuda,blend_data.is_cuda)
        # smplx_data = smplx_data.to(self.device)
        # blend_data = blend_data.to(self.device)
       
        if input_image is not None:  
            self.load_models_to_device(["image_encoder", "vae"])
            image_emb = self.encode_image_with_blend(input_image, num_frames, height, width, combine_blend_latents, smplx_mask)
        else:
            image_emb = self.encode_image_with_blend_warpper(clip_context, num_frames, height, width, combine_blend_latents, smplx_mask)
        # Denoise
        ...
        if  self.useunipc:
            ...
            for progress_id in progress_bar_cmd(range(len(self.scheduler.sigmas))):
                ...
                # Inference
                model_input = latents
                noise_pred_posi = model_fn_wan_video(self.dit, model_input, timestep=timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device), **prompt_emb_posi, **image_emb, **extra_input, **tea_cache_posi, add_condition = condition)
                if cfg_scale != 1.0:
                    noise_pred_nega = model_fn_wan_video(self.dit, latents, timestep=timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device), **prompt_emb_nega, **image_emb, **extra_input, **tea_cache_nega)
                    noise_pred = noise_pred_nega + cfg_scale * (noise_pred_posi - noise_pred_nega)
                else:
                    noise_pred = noise_pred_posi
