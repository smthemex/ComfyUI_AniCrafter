...
class ModelManager:
    ...
    def load_lora_v2_unianimate(self, file_path="", state_dict={}, lora_alpha=1.0, is_final = True):
    ...
    def load_lora_v2(self, file_path="", state_dict={}, lora_alpha=1.0, is_final = True):
        ...
        if isinstance(file_path, list):
            ...
            has_key = False
            for key in state_dict.keys():
                if "blend_embedding" in key or "smplx_embedding" in key or "randomref_emb_blend" in key or "randomref_emb_smplx" in key or 'lora' in key:
                    has_key = True
            if has_key:
                print("has lora or conv keys!", file_path)

            
            # extract weights of "dwpose_embedding" for added modules
            for key in state_dict.keys():
                if "blend_embedding" in key or "smplx_embedding" in key or "randomref_emb_blend" in key or "randomref_emb_smplx" in key:
                    self.state_dict_new_module[key] = state_dict[key]
                    # self.state_dict_new_save[key] = state_dict[key]
                    # is_loaded = True
                    # print("Loading Convs Parameters.")
                    # print(file_path)
    ...
    def load_lora_v2_combine(self, file_path="", state_dict={}, lora_alpha=1.0, is_final = True):
        ...
        if isinstance(file_path, list):
            ...
            for file_path_ in file_path:
                ...
                if ii==len_file_path:
                    is_final = True
                self.load_lora_v2_combine(file_path_, state_dict=state_dict, lora_alpha=lora_alpha, is_final = is_final)
                ii += 1
        else:
            print(f"Loading LoRA models from file: {file_path}")
            is_loaded = False
            ...
            has_key = False
            for key in state_dict.keys():
                if "blend_embedding" in key or "smplx_embedding" in key or 'lora' in key:
                    has_key = True
            if has_key:
                print("has lora or conv keys!", file_path)
            ...
            # extract weights of "dwpose_embedding" for added modules
            for key in state_dict.keys():
                if "blend_embedding" in key or "smplx_embedding" in key:
                    self.state_dict_new_module[key] = state_dict[key]
                    # self.state_dict_new_save[key] = state_dict[key]
                    # is_loaded = True
                    # print("Loading Convs Parameters.")
    ...
    def load_lora_v2_zeroconv(self, file_path="", state_dict={}, lora_alpha=1.0, is_final = True):
        ...
        if isinstance(file_path, list):
            ...
            for file_path_ in file_path:
                ...
                if ii==len_file_path:
                    is_final = True
                self.load_lora_v2_zeroconv(file_path_, state_dict=state_dict, lora_alpha=lora_alpha, is_final = is_final)
                ii += 1
        else:
            print(f"Loading LoRA models from file: {file_path}")
            is_loaded = False
            ...
            has_key = False
            for key in state_dict.keys():
                if "blend_embedding" in key or "smplx_embedding" in key or "encode_blend_latent" in key or 'lora' in key:
                    has_key = True
            if has_key:
                print("has lora or conv keys!", file_path)
            ...
            for key in state_dict.keys():
                if "encode_blend_latent" in key:
                    has_key = True
            if has_key:
                print("has encode_blend_latent keys!", file_path)

            
            # extract weights of "dwpose_embedding" for added modules
            for key in state_dict.keys():
                if "blend_embedding" in key or "smplx_embedding" in key or "encode_blend_latent" in key:
                    self.state_dict_new_module[key] = state_dict[key]
                    # self.state_dict_new_save[key] = state_dict[key]
                    # is_loaded = True
                    # print("Loading Convs Parameters.")
        ...
        try:
            ...
            if isinstance(file_path, list):
                ...
                for path in file_path:
                    part_dict = load_state_dict(path)
